"""Table generation orchestrator for the Ghost Bank CIL paper."""
...
def generate_all_tables(
    ...
    """Run all family table generators and report outputs.

    Modules receive the output root (``out_dir``) and place files under
    their own family/type/experiment + tables/ sub-path.
    """
    cfg = get_config()
    ...
    logger.info("=" * 72)
    logger.info("GHOST BANK CIL - TABLE GENERATION")
    logger.info("=" * 72)
    ...
    for family in targets:
        ...
        family_out = module.generate_tables(runs, out_dir)
        outputs += family_out
...
def main() -> None:
    ...
    try:
        generate_all_tables(families=args.families)
    except Exception as exc:
        logger.error("Table generation failed: %s", exc, exc_info=True)
        sys.exit(1)
